...
test('pulls changes if version directory exists', async function (t) {
  ...
  await runTests('some dir', 'some version')
...
test('makes directory and clones if version directory does not exist', async function (t) {
  const {runTests, util} = t.context
  util.exists.rejects()
  util.mkdir.resolves()
  util.exec.resolves(defaultCommandOutput)
...
test('runs test cases', async function (t) {
  const {runTests, util} = t.context
  util.exists.resolves()
  util.exec.resolves(defaultCommandOutput)
...
test('returns 0 on success', async function (t) {
  const {logger, runTests, util} = t.context
  util.exists.resolves()
  util.exec.resolves(defaultCommandOutput)

  const result = await runTests('some dir', 'some version')
  t.same(result, {
...
test('returns err.code on failure', async function (t) {
  const {logger, runTests, util} = t.context
  util.exists.resolves()
  util.exec
...
test('return 1 if err.code is missing on failure', async function (t) {
  const {logger, runTests, util} = t.context
  util.exists.rejects()
